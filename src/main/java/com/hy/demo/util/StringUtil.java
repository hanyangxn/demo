...
public class StringUtil {
	...
	public static String repeat(String text, int count) {
		...
		for (i = 0; i < count; i++) { buf.append(text); }
		return buf.toString();
	}
	...
	public static String[] split(String string, String separator, boolean trim) {
		...
		for (int p : posData) {
			if (trim) { result[index] = string.substring(oldPos, p).trim(); }
			else { result[index] = string.substring(oldPos, p); }
	...
	public static boolean isSame(String string1, String string2) {
		if (string1 != null) { return string1.equalsIgnoreCase(string2); }
		else if (string2 != null) { return string2.equalsIgnoreCase(string1); }
		else { return true; }
	}
	...
	public static boolean isEqual(String string1, String string2) {
		if (string1 != null) { return string1.equals(string2); }
		else if (string2 != null) { return string2.equals(string1); }
		else { return true; }
	}
	...
	public static String toHTML(String text, boolean nbspAsEmpty, boolean allowNewLine) {
		...
		for (i = 0; i < j; i++) {
			...
			switch (c) {
			case ' ':
				if (i < j - 1 && text.charAt(i + 1) == ' ' || i > 1 && text.charAt(i - 1) == ' ') { out.append("&nbsp;"); }
				else { out.append(" "); }
				break;
			case '"':
				out.append("&quot;");
	...
	public static int indexOf(String list[], String string) {
		...
		j = list.length;
		for (i = 0; i < j; i++) {
			if (list[i].equals(string)) { return i; }
		}
		return -1;
	}
	...
	/**
	 * 判断指定的字符串是否为空，空串是指值为null或长度为0。
	 * @param list 需要判断的字符串列表。
	 * @return true为空，false非空。
	 */
	public static boolean isEmpty(List<String> list) {
		boolean rtn = list == null || list.isEmpty() || list.contains(null);
		if(!rtn) {
			for(String item:list) {
				rtn = rtn || (item.trim().length() == 0);
				if(rtn) break;
			}
		}
		return rtn;
	...
	public static String getBlank(Object str) {
		return str == null ? "" : (str instanceof String[] ? (((String[])str).length == 0 ? "" : String.join(";", ((String[])str))) : str.toString());
	}
}